	...
	real*8, parameter :: time=1800.d0, t0=time-60		!! record last 1 min for length distribution
	...
	real*8,parameter :: rhop = 5.d0				!! profilin density in micro-Molar
...
!	real*8,parameter :: rhof = 5.d0				!! formin density in nano-Molar
	...
	real*8,parameter :: kp0 = 11.6d0/vton, km0 = 1.4d0		!! polymerization in barbed end : Fujiwara et al PNAS-2011
	real*8,parameter :: k1p = 35.7d0/vton, k1m = 1.63d8		!! dimer formation : reaction-b Sept&McCammon BPJ-2001
	real*8,parameter :: k2p = 2.18d0/vton, k2m = 1.3d3		!! trimer formation : reaction-f Sept&McCammon BPJ-2001
	real*8,parameter :: kpp = 40.0d0/vton, kpm = 0.75d0		!! profilin-monomer reaction : Courtemanche&Pollard Biochem. 2013
	real*8,parameter :: kpp0 = 11.d0/vton, kpm0 = 1450.d0		!! profilin-actin -filament reaction : Funk et al Elife 2019
	real*8,parameter :: kpam0 = 50.d0				!! profilin-actin -filament reaction : Funk et al Elife 2019
	real*8,parameter :: kfp0=29.1d0/vton, kfm0=8.1d-5			!! formin filament reactions : Shekhar et al Nt.Comm. 2019
	real*8,parameter :: kcp0=12.8d0/vton, kcm0=2.d-4			!! capping reactions : Shekhar et al Nt.Comm. 2019
	real*8,parameter :: kfcp0=0.21d0/vton, kfcm0=1.8d-3, kfcm20=4.2d-3	!! cap-formin interaction : Shekhar et al Nt.Comm. 2019
	real*8,parameter :: kcfp0=1.6d0/vton, kcfm0=6.2d-3, kcfm20=6.2d-3	!! cap-formin interaction : Shekhar et al Nt.Comm. 2019
	real*8,parameter :: factor1=1.d0					!! formin: barbed end growth by only actin (mdia1 - 1) (mdia2 - 0.2)
	real*8,parameter :: factor2=6.d0					!! formin: barbed end growth by profilin-actin (mdia1 - 6) (mdia2 - 1.5)
	real*8,parameter :: factor3=3.d0					!! profilin unbinding from formin bound barbed end
        
	real*8,parameter :: kfnuc=1.d-5/(vton*vton)				!! formin mediated nucleation from dimer : Zweifel BPJ 2021(2.d-4/(vton*vton))
        
	integer,parameter :: nf_max=5000					!! Maximum number of filaments allowed. nf should be < nf_max		
...
!	integer, parameter :: Nc=nint(rhoc*V*vton/1000.d0)	!! capping pool
	...
	integer, parameter :: nstep=nint(time)			!! record timeseries data once per second
...
!---------------------------------------------------
	...
	real*8 :: tau
	real*8 :: nm, nd, np, npa, dnm, dnd, dnp, dnpa
	...
	integer :: l5, lp5, lpa
	integer :: nf
	integer :: nnt, Rid				! Rid = reaction id, useful to detect error
...
!------------------------------------------------------
	...
	open(unit=500,file='ran.txt',status='unknown')			!! random seed
	open(unit=900,file='log.txt',status='unknown')			!! log of important reactions
...
!------- using rescaled time t ~ t*k1m
	...
	np=p
	do k=1,1000
	dnd = ( (k1p/k1m)*(nm*(nm-1))/V - nd )*dt
	dnpa = ( (kpp/k1m)*(nm*np)/V - (kpm/k1m)*npa )*dt
	dnp = ( (kpp/k1m)*(nm*np)/V - (kpm/k1m)*npa )*dt
	dnm = 2.d0*( (k1p/k1m)*(nm*(nm-1))/V - nd )*dt + ( (kpp/k1m)*(nm*np)/V - (kpm/k1m)*npa )*dt
	nd = nd + dnd
	npa = npa + dnpa
	np = np - dnp
	nm = nm - dnm
	...
	if(pa+p.ne.Np0) then
	write(*,*) '*** ERROR *** PROFILIN CONSERVATION', pa, p, pa+p, Np0
	GOTO 100
	endif
...
!----------------- pass propensity sum as beta_sum -------------------
...
!----------------- write ---------------------
	...
	write(100,*) t, nf, float(sum(l))/nf, Nc-cap, p, pa, Nfr-frm, rcount,m
	nk=nk+1
	endif
